...
**Resolution applied:** fixed at the source, not as a DL-safe-rules-specific post-processing step -- `FormulaToOWL.next()` (`alo_translator/serializers/owl.py`) now emits `ObjectSomeValuesFrom(succ, ...)` instead of `ObjectAllValuesFrom(succ, ...)` unconditionally, since the two are logically equivalent for this property. **Scope check, not overclaimed:** this is a confirmed fix for the isolated single-rule case above (converting only `outcome_m_h1_q` to a rule). For the existing Konclude/plain-axiom pipeline, it is logically neutral, not an improvement -- verified the `SomeValuesFrom` and `AllValuesFrom` forms give the identical correct 14/14 result via Konclude on the same real file. It also did NOT resolve the full 19-query DL-safe-rules test (see the second anomaly below) -- applying it produced a byte-for-byte identical result to before the fix on that test, so it is not, by itself, "progress" on making the rules path usable end to end.
