...
**Not yet done:** verifying this `oneOf` encoding still produces logically-correct results (not just "runs fast") when combined with real query axioms/rules on top -- confirm it doesn't change what's entailed before treating it as a real replacement for `EquivFullCardinalityStrategy`'s cardinality-based approach. Also not yet checked: whether the full `alo_translator/serializers/index_strategies.py` `EquivFullCardinalityStrategy` needs a matching new strategy class implementing this encoding, or whether this can be verified as a drop-in swap.
