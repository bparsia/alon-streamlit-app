...
**Status: WORKAROUND AVAILABLE, root cause UNRESOLVED.** `dlsafe_rules.py` should be updated to always exclude `outcome_*` classes from rule conversion (see workaround above) before being considered usable. Once that exclusion is added, re-verify against the full 14-query known-good baseline end to end (not just this one isolated inlined test) before calling the HermiT/DL-safe-rules pipeline production-ready.
