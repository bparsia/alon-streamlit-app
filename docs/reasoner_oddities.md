...
**Not yet done:** isolating whether the query-axiom bottleneck (now that the structural one is fixed) is present with the DL-safe-rules translation too, or only with plain axioms; whether it's a subset of the 91 queries or genuinely needs all of them; and whether `EquivChainedNominalStrategy`'s results are correctness-verified against the known-good baseline for a case that actually completes (e.g. re-verify 3.1/isabella-scale models still give correct results under this strategy, not just "runs fast").
