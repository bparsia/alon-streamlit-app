...
def write_table_translations(rows, path: Path):
    lines = [
        ...
        r"\hline",
        r"Model & \multicolumn{4}{c}{pyDatalog} & \multicolumn{4}{c}{OWL / Konclude} \\",
        r" & Facts & Rules & Preds. & Time (s) & Axioms & Classes & Indiv. & Time (s) \\",
        r"\hline",
    ]
    for r in rows:
        d, o = r["datalog"], r["owl"]
        lines.append(
            f"{tex_escape(r['name'])} & {d['facts']} & {d['rules']} & {d['predicates']} & {r['datalog_time']:.3f} & "
            f"{o['axioms']} & {o['classes']} & {o['individuals']} & {r['owl_time']:.3f} \\\\"
        )
    lines += [r"\hline", r"\end{tabular}", ""]
    path.write_text("\n".join(lines))
